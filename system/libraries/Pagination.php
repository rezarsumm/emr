...
class CI_Pagination {
        ...
        var $cur_tag_open		= '<li><a href="#" class="active">';
	...
	function create_links()
	{
        ...
        function create_links_with_sorting($params_sorting = "")
	{
		// If our item count or per-page total is zero there is no need to continue.
		if ($this->total_rows == 0 OR $this->per_page == 0)
		{
		...
		{
			$first_url = ($this->first_url == '') ? $this->base_url : $this->first_url;
			$output .= $this->first_tag_open.'<a '.$this->anchor_class.'href="'.$first_url.''.$params_sorting.'">'.$this->first_link.'</a>'.$this->first_tag_close;
		}
		...
		if  ($this->prev_link !== FALSE AND $this->cur_page != 1)
		{
			...
			if ($i == 0 && $this->first_url != '')
			{
				$output .= $this->prev_tag_open.'<a '.$this->anchor_class.'href="'.$this->first_url.''.$params_sorting.'">'.$this->prev_link.'</a>'.$this->prev_tag_close;
			}
			else
			{
				$i = ($i == 0) ? '' : $this->prefix.$i.$this->suffix;
				$output .= $this->prev_tag_open.'<a '.$this->anchor_class.'href="'.$this->base_url.$i.''.$params_sorting.'">'.$this->prev_link.'</a>'.$this->prev_tag_close;
			}
			...
			for ($loop = $start -1; $loop <= $end; $loop++)
			{
				...
				if ($i >= 0)
				{
					...
					else
					{
						...
						if ($n == '' && $this->first_url != '')
						{
							$output .= $this->num_tag_open.'<a '.$this->anchor_class.'href="'.$this->first_url.''.$params_sorting.'">'.$loop.'</a>'.$this->num_tag_close;
						}
						else
						{
							$n = ($n == '') ? '' : $this->prefix.$n.$this->suffix;

							$output .= $this->num_tag_open.'<a '.$this->anchor_class.'href="'.$this->base_url.$n.''.$params_sorting.'">'.$loop.'</a>'.$this->num_tag_close;
						}
					}
				}
			}
		...
		if ($this->next_link !== FALSE AND $this->cur_page < $num_pages)
		{
			$output .= $this->next_tag_open.'<a '.$this->anchor_class.'href="'.$this->base_url.$this->prefix.($this->cur_page * $this->per_page).$this->suffix.''.$params_sorting.'">'.$this->next_link.'</a>'.$this->next_tag_close;
		}
		...
		{
			$i = (($num_pages * $this->per_page) - $this->per_page);
			$output .= $this->last_tag_open.'<a '.$this->anchor_class.'href="'.$this->base_url.$this->prefix.$i.$this->suffix.''.$params_sorting.'">'.$this->last_link.'</a>'.$this->last_tag_close;
		}
