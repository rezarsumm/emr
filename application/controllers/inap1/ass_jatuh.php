...
class ass_jatuh extends ApplicationBase {
  ...
  public function delete_process($FS_KD_TRS = "") {
    ...
    $data = $this->m_rawat_jalan->get_data_jatuh_by_id(array($FS_KD_TRS));
    $riwayat = array( 
       $data['FS_KD_REG'],
       $data['FS_KD_TRS'],  
       $data['FS_TIPE_RISIKO_JATUH'], 
       $this->com_user['user_name'],
       date('Y-m-d H:i:s'), 
    );

    $FS_KD_TRS=  $data['FS_KD_TRS'];

    $FS_KD_REG=  $data['FS_KD_REG'];

    // insert
    $this->m_rawat_jalan->insert_riwayat_jatuh($riwayat);

    if ($this->m_rawat_jalan->delete_ases_jatuh2($FS_KD_TRS)) {  
        $this->tnotification->sent_notification("success", "Data berhasil dihapus");
        // default redirect
        redirect("inap/ass_jatuh/add/" . $FS_KD_REG);
    } else {
        // default error
