...
class m_cppt extends CI_Model {
    ...
    function insert_status($params) {
      ...
      function INSERT_LAP($params) {
        $sql = "INSERT INTO PKU.dbo.LAPORAN_OPERASI( FS_KD_REG, KD_AHLI_ANESTESI, KD_ANESTESI, DIAGNOSA_PRE, DIAGNOSA_POST, BAGIAN_EKSISI, KIRIM_PA, TGL_MULAI, JAM_MULAI, JAM_SELESAI, LAMA_OP, URAIAN_LAPORAN, UMUR_SAAT_OP, KD_OPERATOR, KD_ASISTEN, KD_PERAWAT, NAMA_OPERASI)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        return $this->db->query($sql, $params);
    }
    ...
    function INSERT_PASCA_BEDAH($params) {
        $sql = "INSERT INTO PKU.dbo.RENCANA_MEDIS_POST_OP( FS_KD_REG, TGL_OP, TINGKAT_PERAWATAN, PERIODE_MONITOR, KONSUL_LAYANAN_LAIN, TERAPI, mdb)
        VALUES(?,?,?,?,?,?,?)";
        return $this->db->query($sql, $params);
    }
