...
// load model
   ...
   public function cetak_rujukan_lab_ranap($FS_KD_REG = "", $FS_KD_TRS ="") { 
        $this->_set_page_rule("R");
        $this->load->library('html2pdf');
        $now = date('Y-m-d');
        $data['rs_pasien'] = $this->m_rawat_jalan->get_px_by_dokter_by_rg2(array($FS_KD_REG));
        $data["result"] = $this->m_rawat_jalan->get_data_medis_by_rg2(array($FS_KD_REG, $FS_KD_TRS));
        $data["rs_lab"] = $this->m_rawat_jalan->get_radiologi_ranap(array($FS_KD_REG));
        $data["header1"] = $this->m_rawat_jalan->get_header1();
        $data["header2"] = $this->m_rawat_jalan->get_header2();
        $data["alamat"] = $this->m_rawat_jalan->get_alamat();
        ob_start();
        $this->load->view('rm/rawat_jalan/rujukan_penunjang_lab_ranap', $data);
        $content = ob_get_contents();
        ob_end_clean();
   ...
   public function cetak_rujukan_rad_ranap($FS_KD_REG = "", $FS_KD_TRS ="") { 
        $this->_set_page_rule("R");
        $this->load->library('html2pdf');
        $now = date('Y-m-d');
        $data['rs_pasien'] = $this->m_rawat_jalan->get_px_by_dokter_by_rg2(array($FS_KD_REG));
        ...
        $data["alamat"] = $this->m_rawat_jalan->get_alamat();
        ob_start();
        $this->load->view('rm/rawat_jalan/rujukan_penunjang_rad_ranap', $data);
        $content = ob_get_contents();
        ob_end_clean();
