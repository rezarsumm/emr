...
class Ews_hamil extends ApplicationBase {
    ...
    public function add_process() { 
       ...
       public function delete($idews = "", $FS_KD_REG = "") {
        ...
        $data = $this->m_rawat_jalan->get_data_ews_hamil(array($idews));
        $riwayat = array(
           $data['id'],
           $data['FS_KD_REG'], 
           $data['TGL'], 
        ...
        // insert
        $this->m_rawat_jalan->insert_riwayat_ews_hamil($riwayat);

        if ($this->m_rawat_jalan->delete_ews_hamil($idews)) {  
            $this->tnotification->sent_notification("success", "Data berhasil dihapus");
            // default redirect
            redirect("inap/ews_hamil/add/" . $FS_KD_REG);
        } else {
            // default error
