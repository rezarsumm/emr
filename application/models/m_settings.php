...
class m_settings extends CI_Model {
    ...
    function get_total_data_portal() {
        $sql = "SELECT COUNT(*)'total' FROM PKU.dbo.tac_com_portal";
    ...
    function get_all_portal() {
        $sql = "SELECT * FROM PKU.dbo.tac_com_portal";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
    ...
    function get_portal_by_id($portal_id) {
        $sql = "SELECT * FROM PKU.dbo.tac_com_portal WHERE portal_id = ?";
    ...
    function insert_portal($params) {
        $sql = "INSERT INTO PKU.dbo.tac_com_portal (portal_nm, site_title, site_desc, meta_desc, meta_keyword, mdb, mdd)
                VALUES (?, ?, ?, ?, ?, ?, GETDATE())";
    ...
    function update_portal($params) {
        $sql = "UPDATE PKU.dbo.tac_com_portal SET portal_nm = ?, site_title = ?, site_desc = ?, meta_desc = ?, meta_keyword = ?,
                mdb = ?, mdd = GETDATE()
                WHERE portal_id = ?";
        return $this->db->query($sql, $params);
    }
    ...
    function delete_portal($params) {
        $sql = "DELETE FROM PKU.dbo.tac_com_portal WHERE portal_id = ?";
        return $this->db->query($sql, $params);
    }
    ...
    function get_all_roles() {
        $sql = "SELECT b.portal_nm, a.* 
                ...
                INNER JOIN PKU.dbo.tac_com_portal b ON a.portal_id = b.portal_id
                WHERE a.role_id <> 1
    ...
    function insert_role($params) {
        $sql = "INSERT INTO PKU.dbo.tac_com_role (portal_id, role_nm, role_desc, default_page)
                VALUES (?, ?, ?, ?)";
        return $this->db->query($sql, $params);
    }
    ...
    function update_role($params) {
        $sql = "UPDATE PKU.dbo.tac_com_role SET portal_id = ?, role_nm = ?, role_desc = ?, default_page = ?
                WHERE role_id = ?";
        return $this->db->query($sql, $params);
    }
    ...
    function delete_role($params) {
        $sql = "DELETE FROM PKU.dbo.tac_com_role WHERE role_id = ?";
        return $this->db->query($sql, $params);
    }

    // insert role menu
    function insert_role_menu($params) {
        $sql = "INSERT INTO PKU.dbo.tac_com_role_menu (role_id, nav_id, role_tp) VALUES (?, ?, ?)";
        return $this->db->query($sql, $params);
    }

    // delete role menu
    function delete_role_menu($params) {
        $sql = "DELETE FROM PKU.dbo.tac_com_role_menu WHERE role_id = ?";
        return $this->db->query($sql, $params);
    }
    ...
    function get_all_portal_menu() {
        $sql = "SELECT a.portal_id,a.portal_nm,a.site_title, COUNT(b.nav_id)'total_menu' 
                FROM PKU.dbo.tac_com_portal a
                LEFT JOIN PKU.dbo.tac_com_menu b ON a.portal_id = b.portal_id
    ...
    function get_all_menu_by_parent($params) {
        $sql = "SELECT * FROM PKU.dbo.tac_com_menu
    ...
    function get_all_menu_selected_by_parent($params) {
        $sql = "SELECT a.*, b.role_id, b.role_tp
                FROM PKU.dbo.tac_com_menu a
                LEFT JOIN (SELECT * FROM PKU.dbo.tac_com_role_menu WHERE role_id = ?) b ON a.nav_id = b.nav_id
    ...
    function get_detail_menu_by_id($id_role) {
        $sql = "SELECT * FROM PKU.dbo.tac_com_menu WHERE nav_id = ?";
        $query = $this->db->query($sql, $id_role);
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
            $query->free_result();
    ...
    function insert_menu($params) {
        $sql = "INSERT INTO PKU.dbo.tac_com_menu (portal_id, parent_id, nav_title, nav_desc, nav_url, nav_no, active_st, display_st, mdb, mdd)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, GETDATE())";
        return $this->db->query($sql, $params);
    }
    ...
    function update_menu($params) {
        $sql = "UPDATE PKU.dbo.tac_com_menu
    ...
    function update_icon($params) {
        $sql = "UPDATE PKU.dbo.tac_com_menu SET nav_icon = ? WHERE nav_id = ?";
        return $this->db->query($sql, $params);
    }

    // delete menu
    function delete_menu($params) {
        $sql = "DELETE FROM PKU.dbo.tac_com_menu WHERE nav_id = ?";
        return $this->db->query($sql, $params);
    }

    // update parent
    function update_parent($params) {
        $sql = "UPDATE PKU.dbo.tac_com_menu SET parent_id = ? WHERE parent_id = ?";
        return $this->db->query($sql, $params);
    }
    ...
    function get_all_users($params) {
        $sql = "SELECT *
                ...
                INNER JOIN PKU.dbo.tac_com_role_user b ON a.user_id = b.user_id
                INNER JOIN PKU.dbo.tac_com_role c ON b.role_id = c.role_id
                INNER JOIN DB_RSMM.dbo.TUSER d ON a.user_name = d.NamaUser";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
            $query->free_result();
    ...
    function is_exist_username($username) {
        $sql = "SELECT COUNT(*)'total' FROM PKU.dbo.tac_com_user WHERE user_name = ?";
    ...
    function insert_user($params) {
        $sql = "INSERT INTO PKU.dbo.tac_com_user (user_name, user_pass, user_key, lock_st, mdb, mdd) VALUES ( ?, ?, ?, ?, ?, GETDATE())";
        return $this->db->query($sql, $params);
    }

    // insert user detail
    function insert_user_detail($params) {
        $sql = "INSERT INTO PKU.dbo.tac_users (user_id, nama_lengkap, alamat, no_telp, mdb,mdd) VALUES (?, ?, ?, ?, ?, GETDATE())";
        return $this->db->query($sql, $params);
    }

    // update user
    function update_user($params) {
        $sql = "UPDATE PKU.dbo.tac_com_user SET user_name = ?, user_pass = ?, user_key = ?, lock_st = ?, user_mail = ?,
                mdb = ?, mdd = GETDATE() 
                WHERE user_id = ?";
        return $this->db->query($sql, $params);
    }
    ...
    function update_user_detail($params) {
        $sql = "UPDATE PKU.dbo.tac_users SET nama_lengkap = ?, alamat = ?, no_telp = ?, mdb = ?, mdd = GETDATE() 
                WHERE user_id = ?";
        return $this->db->query($sql, $params);
    }
    ...
    function insert_role_user($params) {
        $sql = "INSERT INTO PKU.dbo.tac_com_role_user (role_id, user_id)                 VALUES (?, ?)";
        return $this->db->query($sql, $params);
    }

    // update role user
    function update_role_user($params) {
        $sql = "UPDATE PKU.dbo.tac_com_role_user SET role_id = ? WHERE user_id = ?";
        return $this->db->query($sql, $params);
    }
