...
class jiwa extends ApplicationBase {
...
// load model
        ...
        $this->smarty->assign('m_surat', $this->m_surat);
...
// set template content
        $this->smarty->assign("template_content", "surat/jiwa/index.html");
        ...
        $search = $this->tsession->userdata('surat_medis_sehat');
        if (!empty($search)) {
...
// get search parameter
        ...
        $this->smarty->assign("rs_pasien", $this->m_surat->get_px_by_dokter_wait(array($FD_TGL_MASUK, $FS_KD_PEG, $FS_KD_PEG, $FS_KD_PEG)));
...
// output
    ...
    public function proses_cari() {
        ...
        if ($this->input->post('save') == "Reset") {
            $this->tsession->unset_userdata("surat_medis_sehat");
        } else {
            $params = array(
                "FD_TGL_MASUK" => $this->input->post("FD_TGL_MASUK"),
                "FS_KD_PEG" => $this->input->post("FS_KD_PEG")
            );
            $this->tsession->set_userdata("surat_medis_sehat", $params);
        }
        // redirect
        redirect("surat/jiwa");
    }
...
// set page rules
        $this->_set_page_rule("R");
// set template content
        $this->smarty->assign("template_content", "surat/jiwa/add.html");
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        $this->smarty->load_javascript('resource/js/jquery/select2.js');
        $this->smarty->load_javascript('resource/js/jquery/jquery-ui-timepicker-addon.js');
        // load style
...
// output
     ...
     public function add_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            $data_surat = $this->m_surat->get_no_surat();
            ...
            // insert
            if ($this->m_surat->insert($params)) {
                $this->m_surat->update_tz_parameter_no_surat(array($NOSURAT2));
                // notification
...
// output
   ...
   public function surat_jiwa($FS_KD_REG = "", $FS_KD_TRS = "") {
        $this->_set_page_rule("R");
        $this->load->library('html2pdf');
        $now = date('Y-m-d');
        $data['rs_pasien'] = $this->m_rawat_jalan->get_px_by_dokter_by_rg2(array($now, $FS_KD_REG));
        $data['rs_surat'] = $this->m_surat->get_surat_by_trs(array($FS_KD_TRS));
        ob_start();
        $this->load->view('surat/jiwa/jiwa', $data);
