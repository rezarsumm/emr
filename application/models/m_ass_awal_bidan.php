...
class m_ass_awal_bidan extends CI_Model {
    ...
    function insert_vs($params) {
     ...
     function insert_hamil($params) {
        $sql = "INSERT INTO PKU.dbo.TAC_RI_RIWAYAT_HAMIL(NO_MR, TGL_PARTUS, TEMPAT_PARTUS, USIA_HAMIL, JENIS_LAHIRAN, PENOLONG_LAHIRAN, PENYULIT,
        ANAK_JK_BB, KEADAAN_ANAK_NOW, mdb, mdd)
        VALUEs (?,?,?,?,?,?,?,?,?,?,?)";
        return $this->db->query($sql, $params);
    }
    ...
    function insert_ases_bidan($params) {
        $sql = "INSERT INTO PKU.dbo.TAC_RI_ASES_AWAL_BIDAN_INAP(FS_KD_REG, FS_RIW_PENYAKIT_DAHULU, 
        FS_RIW_PENYAKIT_DAHULU2, 
        FS_STATUS_PSIK,FS_STATUS_PSIK2,FS_HUB_KELUARGA,FS_ST_FUNGSIONAL,FS_AGAMA,FS_NILAI_KHUSUS,
        FS_NILAI_KHUSUS2,FS_PEMERIKSAAN_FISIK,
        FS_RIW_IMUNISASI,FS_RIW_IMUNISASI_KET,FS_RIW_TUMBUH,
        FS_RIW_TUMBUH_KET,
        FS_ANAMNESA,FS_HAID_MEN,FS_HAID_SIKLUS,
        FS_HAID_LAMA,FS_HAID_HPHT,FS_HAID_HPL,FS_HAID_KELUHAN,FS_STATUS_PERKAWINAN,FS_LAMA_MENIKAH,RIWAYAT_KEHAMILAN,
        FS_ASMA_MULAI,FS_ASMA_TERAPI,FS_JANTUNG_MULAI,FS_JANTUNG_TERAPI,FS_DIABETES_MULAI,FS_DIABETES_TERAPI,
        FS_HIPERTENSI_MULAI,FS_HIPERTENSI_TERAPI,FS_SAKIT_LAIN,FS_RIWAYAT_GYNEKOLOGI,FS_RIWAYAT_KB,
        FS_RIWAYAT_KOMPLIKASI_KB,POLA_MAKAN,POLA_MINUM,POLA_BAK,POLA_BAB,POLA_TIDUR,JAM_TERAKHIR_MAKAN,
        JAM_TERAKHIR_MINUM,JAM_TERAKHIR_BAK,JAM_TERAKHIR_BAB,JAM_TERAKHIR_TIDUR,RUMAH_MILIK,TINGGAL_BERSAMA,
        PJ_DARURAT,HUBUNGAN_PJ,AKTIFITAS,SOSIAL_SUPPORT,PENERIAMAAN_PERSALINAN,KEADAAN_UMUM,ALAT_BANTU,MATA,
        KEPALA,TELINGA,HIDUNG,TENGGOROKAN,LEHER,DADA,JANTUNG,PARU_PARU,ABDOMEN,BADAN_GERAK_ATAS,BADAN_GERAK_BAWAH,
        SCLERA,KONJUNGTIVA,CEK_DADA,INSPEKSI_ABDOMEN,LEOPOD_1,LEOPOD_2,LEOPOD_3,LEOPOD_4,AUSKULTASI,KONTRAKSI,
        DURASI_KONTRAKSI,INSPEKSI_ANO_GENITAS,VAGINA_TOUCHER,MASALAH_KEBIDANAN,DIAGNOSA,NAMA_SUAMI,TGL_LAHIR_SUAMI,
        AGAMA_SUAMI,PENDIDIKAN_SUAMI,JOB_SUAMI,mdb,mdd)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,
        ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        return $this->db->query($sql, $params);
    }
    ...
    function update_ases($params) {
        $sql = "UPDATE PKU.dbo.TAC_RI_ASES_AWAL_BIDAN_INAP SET
    ...
    function cek_ass_awal($params) {
        $sql = "SELECT FS_KD_REG FROM PKU.dbo.TAC_RI_ASES_AWAL_BIDAN_INAP WHERE FS_KD_REG = ?";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
    ...
    function get_pasien_bangsal($params) {
        $sql = "SELECT A.NO_REG,A.NO_MR,A.TGL_MULAI,B.NAMA_PASIEN,A.KODE_RUANG, B.JENIS_KELAMIN,B.ALAMAT,D.KODE_BANGSAL,C.NAMA_RUANG,D.NAMA_BANGSAL,E.KODEREKANAN,F.NAMAREKANAN 
            ...
            WHERE A.NO_MR=B.NO_MR AND A.NO_REG=E.NO_REG AND E.KODEREKANAN=F.KODEREKANAN AND A.KODE_RUANG=C.KODE_RUANG AND C.KODE_BANGSAL=D.KODE_BANGSAL  ORDER BY D.KODE_BANGSAL,A.TGL_MULAI
        ";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
    ...
    function get_riw_hamil($params) {
        $sql = "select A.*, B.No_Reg, C.Nama_Pasien
              from PKU.dbo.TAC_RI_RIWAYAT_HAMIL as A, PENDAFTARAN AS B, REGISTER_PASIEN AS C
              WHERE A.NO_MR=B.No_MR and B.No_MR=C.No_MR and B.No_Reg=?";
      $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
            $query->free_result();
  ...
  function delete_anak($params) {
        $sql = "DELETE FROM PKU.dbo.TAC_RI_RIWAYAT_HAMIL WHERE FS_KD_RIWAYAT_HAMIL = ?";
        return $this->db->query($sql, $params);
    }
    ...
    function get_data_ases2_by_rg($params) {
        $sql = "SELECT *
        FROM PKU.dbo.TAC_RI_ASES_AWAL_BIDAN_INAP
        WHERE FS_KD_REG = ?";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
