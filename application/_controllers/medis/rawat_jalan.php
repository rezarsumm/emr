...
// get search parameter
        ...
        //$this->smarty->assign("rs_pasien2", $this->m_rawat_jalan->get_px_by_dokter_wait(array($now,$FS_KD_PEG)));
...
// output
    ...
    public function add_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            //if ($this->m_rawat_jalan->insert_medis($params)) {
              //  $update_kp = $this->m_rawat_jalan->update_tz_parameter_no_kp($NOKP2);
                ...
                $this->m_rawat_jalan->insert_tac_rj_medis($params2);*/
                ...
                if (!empty($lab)) {
                    foreach ($lab as $key => $value) {
                        $this->m_rawat_jalan->insert_pemeriksaan_lab(array($NOKP, $key, $value));
                    }
                }
                ...
                if (!empty($rad)) {
                    foreach ($rad as $key => $value) {
                        $this->m_rawat_jalan->insert_pemeriksaan_rad(array($NOKP, $key, $value));
                    }
                }
                // notification
        ...
        if ($this->input->post('FS_CARA_PULANG') == '2') {
            ...
            redirect("medis/rawat_inap/add/" . $this->input->post('FS_KD_REG'));
        } else {
            redirect("medis/rawat_jalan/cetak/" . $this->input->post('FS_KD_REG') . '/' . $FS_KD_TRS);
        }
    }
    ...
    public function edit_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_rawat_jalan->update_medis($params)) {
                ...
                $lab = $this->input->post('tujuan');
                $this->m_rawat_jalan->delete_pemeriksaan_lab($this->input->post('FS_KD_KP'));
                if (!empty($lab)) {
                    foreach ($lab as $key => $value) {
                        $this->m_rawat_jalan->insert_pemeriksaan_lab(array($this->input->post('FS_KD_KP'), $key, $value));
                    }
                }
                $rad = $this->input->post('tembusan');
                $this->m_rawat_jalan->delete_pemeriksaan_rad($this->input->post('FS_KD_KP'));
                if (!empty($rad)) {
                    foreach ($rad as $key=>$value) {
                        $this->m_rawat_jalan->insert_pemeriksaan_rad(array($this->input->post('FS_KD_KP'), $key, $value));
                    }
                }
