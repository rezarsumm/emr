...
class m_account extends CI_Model {
    ...
    function get_user_profil($params) {
        $sql = "SELECT * FROM 
                (
                        ...
                        FROM tac_com_user a
                        INNER JOIN TD_PEG b ON a.user_name = b.FS_KD_PEG
                        INNER JOIN tac_com_role_user c ON a.user_id = c.user_id
                        INNER JOIN tac_com_role d ON c.role_id = d.role_id
                        LEFT JOIN tac_com_user_login e ON a.user_id = e.user_id
                        LEFT JOIN TD_JABATAN f ON b.FS_KD_JABATAN = f.FS_KD_JABATAN
    ...
    function get_user_detail_by_username($params) {
        $sql = "SELECT a.*, c.role_id, c.role_nm, c.default_page
                FROM tac_com_user a
                LEFT JOIN tac_com_role_user b ON a.user_id = b.user_id
                LEFT JOIN tac_com_role c ON b.role_id = c.role_id
    ...
    function save_user_login($user_id, $remote_address) {
        // get today login
        $sql = "SELECT * FROM tac_com_user_login WHERE user_id = ?";
        $query = $this->db->query($sql, array($user_id));
        if ($query->num_rows() > 0) {
            // tidak perlu diinputkan lagi
            return false;
        } else {
            $sql = "INSERT INTO tac_com_user_login (user_id, login_date, ip_address) VALUES (?, NOW(), ?)";
    ...
    function update_user_logout($user_id) {
        // update by this date
        $sql = "UPDATE tac_com_user_login SET logout_date = NOW() WHERE user_id = ?";
    ...
    function get_data_pribadi($params) {
        $sql = "SELECT a.*, c.role_id, c.role_nm, d.user_mail
                FROM users a 
                LEFT JOIN tac_com_role_user b ON a.user_id = b.user_id
                LEFT JOIN (SELECT * FROM tac_com_role WHERE portal_id = 2) c ON b.role_id = c.role_id
                LEFT JOIN tac_com_user d ON d.user_id = a.user_id
                WHERE a.user_id=?
                GROUP BY a.user_id";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
    ...
    function update_email_users($params) {
        $sql = "UPDATE tac_com_user SET user_mail = ?, mdb = ?, mdd = NOW() 
                WHERE user_id = ?";
    ...
    function get_user_account($id) {
        $sql = "SELECT a.*, b.* FROM tac_com_user a
    ...
    function update_account($params) {
        $sql = "SELECT * FROM tac_com_user WHERE user_id = ?";
        ...
        // update 
        $sql = "UPDATE tac_com_user SET user_name = ?, user_pass = ? WHERE user_id = ?";
        return $this->db->query($sql, $params);
    }

    // check username
    function is_exist_username($params) {
        $sql = "SELECT * FROM tac_com_user WHERE user_name = ?";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $query->free_result();
            return true;
