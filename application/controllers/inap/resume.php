...
class resume extends ApplicationBase {
    ...
    public function edit($FS_RG = "") {
        ...
        $tujuan = $this->m_resume->get_diet_by_rg($FS_RG);
            ...
            $diet = "";
            foreach ($tujuan as $key => $value) {
                $diet .= "" . $value['FS_NM_DIET'] . ",";
            }
            

        $diags = $this->m_resume->get_diag_by_rg($FS_RG);
        $diagn = "";
        foreach ($diags as $key => $value) {
            $diagn .= "" . $value['FS_NM_DIAG_SEK'] . ",";
        }


        $tindakan = $this->m_resume->get_tind_by_rg($FS_RG);
        $tindak = "";
        foreach ($tindakan as $key => $value) {
            $tindak .= "" . $value['FS_NM_TIND'] . ",";
        }

        $this->smarty->assign('rs_tujuan', $tujuan_str);

        $obat = $this->m_resume->get_terapi_by_rg($FS_RG);
        $terapi = "";
        foreach ($obat as $key => $value) {
            $terapi .= "" . $value['FS_NM_OBAT'] . " ".$value['FS_JML_OBAT']." ".$value['FS_DOSIS_OBAT']." ".$value['FS_FREK_OBAT'].", ";
        }
          
        $this->smarty->assign('terapi', $terapi);
        $this->smarty->assign('tindak', $tindak); 
        $this->smarty->assign('diagn', $diagn);
        $this->smarty->assign('diet', $diet);

        $tembusan = $this->m_resume->get_indikasi_dirawat_by_rg($FS_RG);

        $indikasi='';
        foreach($tembusan as $t){
            $indikasi = $indikasi.', '.$t['FS_NM_INDIKASI_DIRAWAT'];
        }
        ...
        $this->smarty->assign('indikasi', $indikasi);
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // output
        ...
        if ($this->tnotification->run() !== FALSE) { 
            ...
            if ((empty($kontrol)) || empty($pulang)) {
                $params = array(
                    ...
                    $this->input->post('TGL_EXPIRED_RUJUKAN'),
    ...
    public function edit_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_resume->update($params)) {
                ...
                if($this->com_user['role_nm']=='Dokter'){
                    $perawat=  $this->input->post('PERAWAT_0');
                    $dokter=   $this->com_user['user_name'];
                }
                else     if($this->com_user['role_nm']=='Perawat'){
                    $perawat=   $this->com_user['user_name'];
                    $dokter= $this->input->post('DOKTER_0');

                }

                $riwayat = array(
                    $this->input->post('FS_KD_REG'),
                    $this->input->post('DIET_0'),
                    $this->input->post('DIAG_SEKUNDER_0'),
                    $this->input->post('INDIKASI_RAWAT_0'),
                    $this->input->post('TINDAKAN_0'),
                    $this->input->post('PULANG_TERAPI_0'),
                    $this->input->post('FS_SEBAB_KEMATIAN_0'),  
                    $perawat,   
                    $dokter,         
                    $this->input->post('FS_KELUHAN_UTAMA_0'), 
                    $this->input->post('FS_RIWAYAT_PENYAKIT_0'),
                    $this->input->post('FS_PEMERIKSAAN_FISIK_0'),
                    $this->input->post('FS_PEMERIKSAAN_PENUNJANG_0'),
                    $this->input->post('FS_TERAPI_0'),
                    $this->input->post('FS_HASIL_LAB_0'),
                    $this->input->post('FS_ALERGI_0'),
                    '-',
                    $this->input->post('FS_KD_LAYANAN_0'),
                    $this->input->post('FD_TGL_KONTROL_0'),
                    $this->input->post('FS_JAM_KONTROL_0'),
                    $this->input->post('FS_DIAG_UTAMA_0'),
                    $this->input->post('FS_KEADAAN_UMUM_BAIK_0'),
                    $this->input->post('FS_KEADAAN_UMUM_MASIH_SAKIT_0'),
                    $this->input->post('FS_KEADAAN_UMUM_SESAK_0'),
                    $this->input->post('FS_KEADAAN_UMUM_PUCAT_0'),
                    $this->input->post('FS_KEADAAN_UMUM_LEMAH_0'),
                    $this->input->post('FS_KEADAAN_UMUM_LAIN_0'),
                    ...
                    '0', 
                    $this->input->post('FS_PEM_FISIK1_0'),
                    $this->input->post('FS_PEM_FISIK2_0'),
                    $this->input->post('FS_PEM_FISIK3_0'),
                    $this->input->post('FS_PEM_FISIK4_0'),
                    $this->input->post('FS_PEM_FISIK5_0'),
                    $this->input->post('FS_PEM_FISIK6_0'),
                    $this->input->post('FS_PEM_FISIK7_0'),
                    $this->input->post('FS_PEM_FISIK8_0'),
                    $this->input->post('FS_INSTRUKSI1_0'),
                    $this->input->post('FS_INSTRUKSI2_0'),
                    $this->input->post('FS_INSTRUKSI3_0'),
                    $this->input->post('FS_INSTRUKSI4_0'),
                    $this->input->post('FS_INSTRUKSI5_0'),
                    $this->input->post('FS_SUHU1_0'),
                    $this->input->post('FS_NADI1_0'),
                    $this->input->post('FS_R1_0'),
                    $this->input->post('FS_TD1_0'),
                    $this->input->post('FD_TGL_PULANG_0'),
                    $this->input->post('FS_KD_LAYANAN2_0'),
                    $this->input->post('FD_TGL_KONTROL2_0'),
                    $this->input->post('FS_JAM_KONTROL2_0'),
                    $this->input->post('FS_KET_INDIKASI_0'),
                    $this->input->post('FS_KET_KONTROL_0'),
                    $this->input->post('FS_KET_KONTROL2_0'), 
                    $this->com_user['user_name'],
                    date('Y-m-d H:i:s'),

                );
                $this->m_rawat_jalan->insert_riwayat_resume($riwayat);
