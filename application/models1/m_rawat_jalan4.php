...
//baru
    ...
    function get_data_op($params) {
        $sql = "select * from PKU.dbo.LAPORAN_ANESTESIA A, PKU.dbo.CHECKLIST_KES_OP B WHERE A.FS_KD_REG='22-00001181' AND B.FS_KD_REG=?";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->num_rows();
            $query->free_result();
    ...
    function insert_jatuh($params) {
        $sql = "INSERT INTO PKU.dbo.TAC_RJ_JATUH(FS_KD_REG, FS_CARA_BERJALAN1, FS_CARA_BERJALAN2, FS_CARA_DUDUK, mdd, mdb)
        VALUES (?,?,?,?,?,?)";
        return $this->db->query($sql, $params);
    }
 ...
 function update_tac_rj_prb($params) {
    ...
    function get_dokter($params) {
        ...
        DB_RSMM.dbo.DOKTER
        WHERE JENIS_PROFESI = 'DOKTER UMUM' OR JENIS_PROFESI = 'DOKTER SPESIALIS' 
    ...
    function get_px_by_dokter_wait($params) {
        $sql = "SELECT a.NOMOR,a.NO_MR,b.NAMA_PASIEN,b.ALAMAT,b.NO_MR,d.FS_STATUS,
    ...
    function get_px_by_dokter_wait_dokter($params) {
        $sql = "SELECT a.NOMOR,a.NO_MR,b.NAMA_PASIEN,b.ALAMAT,b.NO_MR,d.FS_STATUS,e.mdb,
    ...
    function get_px_by_farmasi($params) {
        $sql = "SELECT b.NO_REG,a.NAMA_PASIEN,a.NO_MR,a.ALAMAT,A.JENIS_KELAMIN,
    ...
    function get_data_visite($params) {
      ...
      function get_px_history_dokter2($params) {
        $sql = "SELECT TOP 15 A.TANGGAL, A.KODE_RUANG,A.STATUS,A.NO_REG,B.NAMA_PASIEN, B.ALAMAT,B.KOTA,B.PROVINSI,B.TGL_LAHIR,B.JENIS_KELAMIN, I.NAMA_DOKTER,K.SPESIALIS,L.MAX_RG,M.FS_KD_MEDIS,M.FS_KD_TRS
        FROM PENDAFTARAN A
        LEFT JOIN REGISTER_PASIEN B ON  A.NO_MR=B.NO_MR
        LEFT JOIN DOKTER I ON A.KODE_DOKTER=I.KODE_DOKTER
         LEFT JOIN M_SPESIALIS K ON I.SPESIALIS=K.SPESIALIS
       ...
       function get_px_history_dokter4($params) {
        $sql = "SELECT TOP 8 A.TANGGAL,A.STATUS,A.NO_REG,B.NAMA_PASIEN, B.ALAMAT,B.KOTA,B.PROVINSI,B.TGL_LAHIR,B.JENIS_KELAMIN, I.NAMA_DOKTER,K.SPESIALIS,L.MAX_RG,M.FS_KD_MEDIS,M.FS_KD_TRS, N.FS_STATUS, N.FS_KD_REG
        FROM PENDAFTARAN A
        LEFT JOIN REGISTER_PASIEN B ON  A.NO_MR=B.NO_MR
        LEFT JOIN DOKTER I ON A.KODE_DOKTER=I.KODE_DOKTER
         LEFT JOIN M_SPESIALIS K ON I.SPESIALIS=K.SPESIALIS
    ...
    function get_px_resume($params) {
        $sql = "SELECT A.TANGGAL,A.STATUS,A.NO_REG,B.NAMA_PASIEN, B.ALAMAT, B.GOL_DARAH,B.STATUS_NIKAH,B.NAMA_PASANGAN,B.KOTA,B.PROVINSI,B.TGL_LAHIR,B.JENIS_KELAMIN,B.WARGA_NEGARA,B.PEKERJAAN,B.AGAMA,B.NO_TELP, B.HP1,B.HP2,B.KODE_POS,B.EMAIL,B.NAMA_HUB,B.NO_IDENTITAS,B.HUB_PASIEN,B.TELP_RUMAH, B.FS_ALERGI,L.*,
    ...
    function get_px_by_dokter_by_rm($params) {
        $sql = "SELECT a.NAMA_PASIEN,a.NO_MR,a.ALAMAT,JENIS_KELAMIN,
    ...
    function get_px_by_dokter_by_rg2($params) {
        $sql = "SELECT b.NO_REG,a.NAMA_PASIEN, a.NO_MR,a.ALAMAT,A.JENIS_KELAMIN, b.MEDIS,
        a.TGL_LAHIR,c.SPESIALIS,
        b.TANGGAL,b.TGL_KELUAR,b.KODE_DOKTER,a.FS_HIGH_RISK,d.FS_KD_TRS
        FROM DB_RSMM.dbo.REGISTER_PASIEN a
        LEFT JOIN DB_RSMM.dbo.PENDAFTARAN b ON a.NO_MR=b.NO_MR
        LEFT JOIN DB_RSMM.dbo.DOKTER c ON b.KODE_DOKTER=c.KODE_DOKTER
        LEFT JOIN PKU.dbo.TAC_RJ_MEDIS d ON b.NO_REG=d.FS_KD_REG
    ...
    function get_tac_com_parameter_rencana($params) {
      ...
      function get_px_konsul($params) {
        $sql = "SELECT distinct(a.FS_KD_REG),  c.NO_REG, b.NAMA_PASIEN, b.ALAMAT,b.NO_MR, f.NAMA_DOKTER
  ...
  function cek_asesmen_rajal_perawat($params) {
   ...
   function insert_surathamil($params) {
    ...
    function data_surat_sakit($params) {
        $sql = "SELECT d.*, b.NO_REG,a.NAMA_PASIEN, a.NO_MR,a.ALAMAT,A.JENIS_KELAMIN,
    ...
    function cek_asesmen_umum($params) {
        ...
        WHERE FS_KD_REG = ? and FS_KD_MEDIS in('216','217','215','213','202','219','220','221','312','222','223') ";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->num_rows();
            $query->free_result();
