...
class m_cppt extends CI_Model {
    ...
    function get_list_pra_anes_by_rg3($params) {
        $sql = "select top 1 A.*
        from PKU.dbo.ASES_PRA_ANESTESIA A
        WHERE  A.idoperasi= ?";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
    ...
    function INSERT_LAP($params) {
        $sql = "INSERT INTO PKU.dbo.LAPORAN_OPERASI( FS_KD_REG, KD_AHLI_ANESTESI, KD_ANESTESI, DIAGNOSA_PRE, DIAGNOSA_POST, BAGIAN_EKSISI, KIRIM_PA, TGL_MULAI, JAM_MULAI, JAM_SELESAI, LAMA_OP, URAIAN_LAPORAN, UMUR_SAAT_OP, KD_OPERATOR, KD_ASISTEN, KD_PERAWAT, NAMA_OPERASI, idoperasi)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        return $this->db->query($sql, $params);
    }
    ...
    function INSERT_ALAT_HABIS_PAKAI($params) {
        $sql = "INSERT INTO PKU.dbo.ALAT_HABIS_PAKAI( FS_KD_REG, TGL_OP, namabarang, jumlah,idoperasi)
        VALUES(?,?,?,?,?)";
        return $this->db->query($sql, $params);
    }
    ...
    function INSERT_PASCA_BEDAH($params) {
        $sql = "INSERT INTO PKU.dbo.RENCANA_MEDIS_POST_OP( FS_KD_REG, TGL_OP, TINGKAT_PERAWATAN, PERIODE_MONITOR, KONSUL_LAYANAN_LAIN, TERAPI, mdb,idoperasi)
        VALUES(?,?,?,?,?,?,?,?)";
        return $this->db->query($sql, $params);
    }
