...
class rawat_inap extends ApplicationBase {
    ...
    public function index() {
        ...
        if ($role == '6'){
            $this->smarty->assign("rs_pasien", $this->m_rawat_inap->get_pasien_bangsal_admin(array($date, $date2)));
        }else{
        $this->smarty->assign("rs_pasien", $this->m_rawat_inap->get_pasien_bangsal(array($date, $date2,$FS_KD_PEG)));
        }
        // notification
    ...
    public function add_rj($FS_RG = '') {
        // set page rules
        $this->_set_page_rule("C");
        // set template content
        $this->smarty->assign("template_content", "medis/rawat_inap/add_rj.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        $this->smarty->load_javascript('resource/js/jquery/select2.js');
        $this->smarty->load_javascript('resource/js/tinymce/tinymce.min.js');
    ...
    public function edit_rj($FS_RG = "") {
        // set page rules
        $this->_set_page_rule("C");
        // set template content
        $this->smarty->assign("template_content", "medis/rawat_inap/edit_rj.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        $this->smarty->load_javascript('resource/js/jquery/select2.js');
        $this->smarty->load_javascript('resource/js/tinymce/tinymce.min.js');
    ...
    public function add_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            $params = array(
                ...
                $this->input->post('FS_PLANNING_LAB'),
                $this->input->post('FS_PLANNING_RAD'),
    ...
    public function add_process_rj() {
        // set page rules
        $this->_set_page_rule("C");
        // cek input
        $this->tnotification->set_rules('FS_KD_REG', 'KODE REGISTER', 'trim|required');
    ...
    public function edit_process_rj() {
        // set page rules
        $this->_set_page_rule("C");
        // cek input
        $this->tnotification->set_rules('FS_KD_REG', 'NAMA PASIEN', 'trim|required');
