...
class Pascabedah extends ApplicationBase {
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "op/pascabedah/list.html");
        // load javascript
    ...
    public function cari_process() {
        ...
        redirect("op/pascabedah/add/" . $FS_RG2);
        //} elseif ($cek == '1') {
    ...
    public function add($FS_RG = '',$idoperasi='') {
        ...
        $this->_set_page_rule("C");
        // set template content
        $this->smarty->assign("template_content", "op/pascabedah/add.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        $this->smarty->load_javascript('resource/js/jquery/select2.js');
        $this->smarty->load_javascript('resource/js/jquery/jquery-ui-timepicker-addon.js');
        ...
        $this->smarty->assign("idoperasi", $idoperasi);
         ...
         $this->smarty->assign("rs_pasca_op", $this->m_cppt->get_list_pasca_op_by_rg($FS_RG));
    ...
    public function add_process2() { 
        ...
        // process
         ...
         if ($this->tnotification->run() !== FALSE) {
             ...
             $PERIODE1= $this->input->post('PERIODE_MONITOR');
             $PERIODE2= $this->input->post('PERIODE_MONITOR2');
             $PERIODE=$PERIODE1." sampai ".$PERIODE2;

                $params2 = array(
                    $this->input->post('FS_KD_REG'),
                    $this->input->post('TGL_OP'),
                    $this->input->post('TINGKAT_PERAWATAN'),
                    $PERIODE,
                    $this->input->post('KONSUL_LAYANAN_LAIN'),
                    $this->input->post('TERAPI'),
                    $this->com_user['user_name'],
                    date('Y-m-d'), 
                    $this->input->post('idoperasi')
                );
                $this->m_cppt->INSERT_PASCA_BEDAH($params2);
        ...
        // process
         ...
         if ($this->tnotification->run() !== FALSE) {
             ...
             $PERIODE=$PERIODE1." sampai ".$PERIODE2;
                ...
                $this->m_cppt->DELETE_RENCANA_POST_OP( $this->input->post('idoperasi'));
    ...
    public function detail($FS_RG2='',$id='') { 
        $cek = $this->m_cppt->cek_rencana_post_op(array($id));
        if ($cek == '0') {
            redirect("op/pascabedah/add/" . $FS_RG2."/".$id);
        } elseif ($cek >= '1') {
            
            redirect("op/pascabedah/edit/" . $FS_RG2."/".$id);
    ...
    public function edit($FS_RG = '',$idoperasi='') {
        ...
        $this->_set_page_rule("C");
        // set template content
        $this->smarty->assign("template_content", "op/pascabedah/edit.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        $this->smarty->load_javascript('resource/js/jquery/select2.js');
        $this->smarty->load_javascript('resource/js/jquery/jquery-ui-timepicker-addon.js');
        ...
        $this->smarty->assign("idoperasi", $idoperasi);
         ...
         $this->smarty->assign("rs_cppt", $this->m_cppt->get_cppt_by_rg($FS_RG));
         $this->smarty->assign("rs_pasca_op", $this->m_cppt->get_list_pasca_op_by_rg($FS_RG));
         $this->smarty->assign("rs_pasca_op3", $this->m_cppt->get_list_pasca_op_by_rg3($idoperasi));
        $this->smarty->assign("rs_resep", $this->m_cppt->get_resep());
        $tgl=date('Y-m-d');
        $this->smarty->assign("rs_lap_anes", $this->m_cppt->get_list_lap_anes_by_rg2($FS_RG));
